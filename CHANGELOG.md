...
- **New skill: `core-status`** — project status report skill ported from upstream `/status` command. Scans SDD workspace (`.github/sdd/`), git state, codebase health, and generates actionable recommendations. Adapted from `.claude/commands/core/status.md` to Copilot CLI SKILL.md format with `.github/` path references.
- **New skill: `agent-router`** — intelligent routing table mapping tasks to specialist agents. Includes model cost optimization strategy and serial/parallel composition hints. Generated by `scripts/generate-agent-router.py`.
- **New skill: `data-engineering-guide`** — consolidated DE entry-point auto-invoked when users discuss data engineering tasks. Routes to 15+ DE agents and 8 data engineering skills. Adapted from `plugin-extras/skills/data-engineering-guide/` with Copilot CLI agent name conventions (`de-*`, `architect-*`, `test-*`).
- **New skill: `sdd-workflow`** — SDD workflow quick-reference auto-invoked when users discuss feature development. Guides through all 5 phases with Copilot CLI skill references. Adapted from `plugin-extras/skills/sdd-workflow/` with `.github/sdd/` paths.
- **`scripts/generate-agent-router.py`** — adapted from upstream generator. Key changes for this fork: agents are in a **flat** `.github/agents/` directory (no category subdirectories); category is derived from filename prefix (`de-spark-engineer` → `de`); model normalization handles `Claude Sonnet 4.5` long-form names; CATEGORIES dict updated for fork prefixes (`de`, `fabric`, `workflow` instead of `data-engineering`, `platform`).
- **`scripts/` directory** at repo root for build tooling (distinct from plugin-copilot/ which ships in the plugin).

### Changed

- Skill count: 31 source skills → 35 (added core-status, agent-router, data-engineering-guide, sdd-workflow)
