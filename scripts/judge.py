...
Reads a target file (or stdin), sends it to a non-Copilot model via OpenRouter,
and returns a PASS/FAIL verdict with reasoning. Designed to be invoked by the
`review-judge` skill — user-initiated, one call per invocation, budget-capped.
...
PHASE_MODEL_DEFAULTS: dict[str, str] = {
    "generic": "openai/gpt-4o-mini",   # standalone review-judge invocations
...
GENERIC_SYSTEM_PROMPT = """You are a senior reviewer providing a SECOND OPINION on output produced by another AI (Copilot). Your job is to catch mistakes Copilot's self-review would miss: hallucinated APIs, wrong invariants, silently broken invariants, insecure patterns, logic errors.
...
DEFINE_SYSTEM_PROMPT = """You are a senior product engineer judging a REQUIREMENTS DOCUMENT (Phase 1 / DEFINE) produced by another AI (Copilot) inside the AgentSpec SDD workflow. Your job is to catch what Copilot's self-review would miss on spec quality.
...
DESIGN_SYSTEM_PROMPT = """You are a senior software architect judging a TECHNICAL DESIGN DOCUMENT (Phase 2 / DESIGN) produced by another AI (Copilot) inside the AgentSpec SDD workflow. Your job is to catch architectural mistakes Copilot's self-review would miss.
...
BUILD_SYSTEM_PROMPT = """You are a senior engineer judging CODE or a BUILD REPORT (Phase 3 / BUILD) produced by another AI (Copilot) inside the AgentSpec SDD workflow. Your job is to catch concrete bugs and correctness issues Copilot's self-review would miss.
...
def call_openrouter(
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "HTTP-Referer": "https://github.com/Arthur1511/agentspec-copilot",
...
def render_markdown(verdict: dict[str, object], target: str, model: str) -> str:
    ...
    out.append("")
    out.append("_Judge is advisory. Copilot is the author; final call is yours._")
    return "\n".join(out) + "\n"
...
def main() -> int:
    ...
    ap.add_argument(
        ...
        default="generic",
        help="SDD phase (tunes system prompt + default model). Use 'generic' for standalone calls.",
    )
    ap.add_argument(
        "--model",
        default=None,
    ...
    ap.add_argument(
        "--context",
        default="General code/content review — flag anything Copilot may have gotten wrong.",
        help="Short description of what Copilot was trying to accomplish",
    )
    ap.add_argument(
        "--strict",
        action="store_true",
        help="Strict mode — exit 1 on FAIL even in advisory flow.",
    )
