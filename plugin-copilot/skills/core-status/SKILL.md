...
description: |
  ...
  <example>
  Context: User wants to know what's happening in the project
  user: "What's the current status of this project?"
  assistant: "I'll use the core-status skill to scan the workspace and generate a status report."
  </example>

  <example>
  Context: Start of a new session
  user: "What are we working on?"
  assistant: "Let me run a status scan to get the current project state."
  </example>
---
...
```
Invoke core-status skill              # Full project status report
core-status "sprint review"           # Status with specific context
```
...
# KB domain alignment
Glob("${COPILOT_PLUGIN_ROOT}/kb/*/index.md")
```
...
- **Agent recommendations**: Based on detected work (e.g., SQL files suggest `de-sql-optimizer`, dbt files suggest `de-dbt-specialist`, Python files suggest `python-code-reviewer`).
- **Health fixes**: Flag stale branches, failing tests, missing documentation, high TODO counts, or uncommitted changes.
...
> {If no active features: "No active SDD features. Use `/brainstorm` or `/define` skill to start."}
...
| Documentation | {README.md exists/missing}, {CHANGELOG.md exists/missing} | |
| Uncommitted changes | {Clean/N files changed} | {list files if < 10} |
| TODOs/FIXMEs | {count} found | Review with grep |
...
| Action | Reason |
|--------|--------|
| `/{skill or command}` | {Why this is relevant right now} |
| `/{skill or command}` | {Why this is relevant right now} |
```
...
This skill runs inline without agent delegation. It uses only Glob, Grep, Read, and shell tools. Typical execution time is under 30 seconds.
