...
1. **`${COPILOT_PLUGIN_ROOT}/agents/<name>.agent.md`** — project-level override (checked first)
2. **`${COPILOT_PLUGIN_ROOT}/agents/<name>.agent.md`** — plugin-bundled agent (fallback)

Files in this `custom/` folder are **not** auto-loaded by name. To create a local override, place your file directly in `${COPILOT_PLUGIN_ROOT}/agents/`, e.g.:

```
${COPILOT_PLUGIN_ROOT}/agents/de-dbt-specialist.agent.md   # overrides the built-in agent
```
